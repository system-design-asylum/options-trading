...
mod tests {
    ...
    seller.add_asset(&Asset::USDT, 1000000.0).unwrap(); // Plenty of USDT but not needed for CALL
        ...
        assert!(result.is_err());
    assert_eq!(result.unwrap_err(), "Insufficient BTC asset balance to cover CALL option");
    }
    ...
    fn test_purchase_option_insufficient_funds() {
        ...
        assert!(result.is_err());
    assert_eq!(result.unwrap_err(), "Buyer doesn't have enough {} balance to purchase option");
    }
